...
export default function HomePage() {
  ...
  async function register(e: React.FormEvent) {
    ...
    try {
      ...
      if (!r.ok) throw new Error(await r.text().catch(() => "Register failed"));
    ...
    <main className="max-w-lg mx-auto px-4 py-10">
      ...
      <form onSubmit={register} className="mt-8 space-y-4 rounded-2xl border bg-white p-6 shadow">
        ...
        <label className="block text-sm">Email
          <input className="mt-1 w-full rounded-xl border px-3 py-2" type="email" value={email} onChange={e=>setEmail(e.target.value)} required />
        </label>
